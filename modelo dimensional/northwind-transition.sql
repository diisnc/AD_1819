...
INSERT INTO northwind_dw.dim_data
	SELECT CAST(CONCAT(YEAR(order_date),MONTH(order_date),DAY(order_date),HOUR(order_date),MINUTE(order_date),SECOND(order_date)) AS UNSIGNED),
    order_date, WEEKDAY(order_date), DAY(order_date), MONTH(order_date), YEAR(order_date), QUARTER(order_date), FLOOR((QUARTER(order_date)/3)+1)
    FROM northwind.orders WHERE order_date IS NOT NULL
	UNION 
	SELECT CAST(CONCAT(YEAR(shipped_date),MONTH(shipped_date),DAY(shipped_date),HOUR(shipped_date),MINUTE(shipped_date),SECOND(shipped_date)) AS UNSIGNED),
    shipped_date, WEEKDAY(shipped_date), DAY(shipped_date), MONTH(shipped_date), YEAR(shipped_date), QUARTER(shipped_date), FLOOR((QUARTER(shipped_date)/3)+1)
    FROM northwind.orders WHERE shipped_date IS NOT NULL
	UNION 
	SELECT CAST(CONCAT(YEAR(paid_date),MONTH(paid_date),DAY(paid_date),HOUR(paid_date),MINUTE(paid_date),SECOND(paid_date)) AS UNSIGNED),
    paid_date, WEEKDAY(paid_date), DAY(paid_date), MONTH(paid_date), YEAR(paid_date), QUARTER(paid_date), FLOOR((QUARTER(paid_date)/3)+1)
